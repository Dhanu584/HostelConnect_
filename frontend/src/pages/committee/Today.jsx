...
const defaultEvents = [
  ...
  { id:4, title:'Hostel Fee Last Date',date:'2025-02-15', time:'All Day',  category:'Admin',    color:'#6366f1', description:'Last date to pay hostel fee. Fine applies after.', postedBy:'Committee' },
];
const mealIcons  = { Breakfast:'☀️', Lunch:'🌤️', Snacks:'🌥️', Dinner:'🌙' };
const mealColors = {
  Breakfast:{ bg:'rgba(251,191,36,0.06)',  border:'rgba(251,191,36,0.15)',  accent:'#fbbf24' },
  Lunch:    { bg:'rgba(34,197,94,0.06)',   border:'rgba(34,197,94,0.15)',   accent:'#22c55e' },
  Snacks:   { bg:'rgba(251,146,60,0.06)',  border:'rgba(251,146,60,0.15)',  accent:'#fb923c' },
  Dinner:   { bg:'rgba(139,92,246,0.06)', border:'rgba(139,92,246,0.15)', accent:'#8b5cf6' },
};
...
const inp = (extra={}) => ({ width:'100%', padding:'9px 12px', borderRadius:'8px', background:'#100818', border:'1px solid rgba(139,92,246,0.2)', color:'#faf5ff', fontSize:'13px', outline:'none', boxSizing:'border-box', fontFamily:"'DM Sans',sans-serif", ...extra });
  ...
  return (
    <>
      <style>{`
        ...
        .covl{position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:200;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)}
        .cmdl{background:#0e0818;border:1px solid rgba(139,92,246,0.2);border-radius:16px;padding:28px;width:100%;max-width:440px;max-height:90vh;overflow-y:auto}
      ...
      <div style={{display:'flex',gap:'20px',height:'calc(100vh - 62px - 56px)',fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{flex:'0 0 410px',display:'flex',flexDirection:'column',minWidth:0}}>
          <div style={{marginBottom:'14px'}} className="cfu">
            <p style={{margin:'0 0 3px',color:'#6b7280',fontSize:'12px',letterSpacing:'1px',textTransform:'uppercase',fontWeight:600}}>View Only</p>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <h2 style={{margin:0,fontFamily:"'Playfair Display',serif",fontSize:'24px',fontWeight:700,color:'#faf5ff'}}>Mess Menu 🍽️</h2>
              <span style={{padding:'3px 10px',borderRadius:'20px',background:'rgba(139,92,246,0.08)',border:'1px solid rgba(139,92,246,0.2)',color:'#c4b5fd',fontSize:'11px',fontWeight:700}}>SET BY RECTOR</span>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'14px'}} className="cfu">
            {days.map(day=>(
              <button key={day} onClick={()=>setSelDay(day)} style={{
                ...
                background:selDay===day?'linear-gradient(135deg,#8b5cf6,#7c3aed)':'rgba(255,255,255,0.05)',
                color:selDay===day?'#fff':'#6b7280',
                boxShadow:selDay===day?'0 0 12px rgba(139,92,246,0.35)':'none',
                outline:day===today&&selDay!==day?'1px solid rgba(139,92,246,0.3)':'none',
              }}>{day.slice(0,3)}{day===today&&<span style={{marginLeft:'4px',fontSize:'8px'}}>●</span>}</button>
            ))}
          </div>

          {/* Meal cards — no edit button */}
              ...
              return (
                <div key={meal} className="cmc cfu" style={{borderRadius:'14px',background:c.bg,border:`1px solid ${c.border}`,overflow:'hidden',animationDelay:`${idx*0.07}s`}}>
                  <div style={{position:'relative',height:'120px',overflow:'hidden'}}>
                    ...
                    {/* Read-only badge */}
                    <div style={{position:'absolute',top:'10px',right:'12px',padding:'3px 8px',borderRadius:'8px',background:'rgba(139,92,246,0.15)',border:'1px solid rgba(139,92,246,0.2)',color:'#c4b5fd',fontSize:'10px',fontWeight:600}}>
                      🔒 Read Only
                    </div>
                  </div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>
          <div style={{marginBottom:'14px',display:'flex',alignItems:'flex-end',justifyContent:'space-between'}} className="cfu">
            <div>
              ...
              <h2 style={{margin:0,fontFamily:"'Playfair Display',serif",fontSize:'24px',fontWeight:700,color:'#faf5ff'}}>Events & Notices 📌</h2>
            </div>
            <button onClick={openNew} style={{padding:'8px 16px',borderRadius:'10px',border:'none',marginBottom:'4px',background:'linear-gradient(135deg,#8b5cf6,#7c3aed)',color:'#fff',fontSize:'13px',fontWeight:600,cursor:'pointer',boxShadow:'0 0 14px rgba(139,92,246,0.3)'}}>+ Add Event</button>
          </div>
          ...
          <div className="csc" style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column',gap:'10px',paddingBottom:'16px'}}>
            {events.sort((a,b)=>new Date(a.date)-new Date(b.date)).map((ev,idx)=>{
              ...
              return (
                <div key={ev.id} className="cec cfu" style={{borderRadius:'14px',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.06)',padding:'16px 18px',position:'relative',overflow:'hidden',boxShadow:'0 2px 10px rgba(0,0,0,0.2)',animationDelay:`${idx*0.06}s`}}>
                  ...
                  <div style={{paddingLeft:'8px'}}>
                    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'10px',marginBottom:'6px'}}>
                      <div style={{flex:1}}>
                        ...
                        <h3 style={{margin:0,color:'#faf5ff',fontSize:'15px',fontWeight:600}}>{ev.title}</h3>
                      </div>
                      <div style={{display:'flex',alignItems:'center',gap:'6px',flexShrink:0}}>
                        ...
                        <button onClick={()=>openEdit(ev)} style={{padding:'4px 8px',borderRadius:'7px',background:'rgba(139,92,246,0.1)',border:'1px solid rgba(139,92,246,0.2)',color:'#c4b5fd',cursor:'pointer',fontSize:'12px'}}>✏️</button>
      ...
      {showEvt && (
        <div className="covl" onClick={()=>setShowEvt(false)}>
          <div className="cmdl" onClick={e=>e.stopPropagation()}>
            <h3 style={{margin:'0 0 20px',color:'#faf5ff',fontFamily:"'Playfair Display',serif",fontSize:'20px'}}>{editEvt?'Edit Event':'Add New Event'}</h3>
            ...
            <div style={{display:'flex',gap:'10px',marginTop:'20px'}}>
              <button onClick={saveEvt} style={{flex:1,padding:'10px',borderRadius:'10px',border:'none',background:'linear-gradient(135deg,#8b5cf6,#7c3aed)',color:'#fff',fontWeight:600,cursor:'pointer',fontSize:'14px'}}>{editEvt?'Save Changes':'Add Event'}</button>
