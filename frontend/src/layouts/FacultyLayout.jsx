...
const facultyNavLinks = [
  ...
  { label: 'Grades',       path: '/faculty/grades',     icon: '📊' },
  { label: 'Materials',    path: '/faculty/materials',  icon: '📁' },
...
export default function FacultyLayout() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", background: '#f0fdf4' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          >
            {[0,1,2].map(i => (
              <span key={i} style={{
                display: 'block', width: '22px', height: '2px', background: '#d1fae5', borderRadius: '2px', transition: 'all 0.3s',
                transform: sidebarOpen ? i === 0 ? 'rotate(45deg) translate(5px, 5px)' : i === 2 ? 'rotate(-45deg) translate(5px, -5px)' : 'scaleX(0)' : 'none',
              }} />
            ))}
          </button>

          <NavLink to="/faculty" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontFamily: "'Syne', sans-serif", fontSize: '20px', fontWeight: 700, color: '#f0fdf4', letterSpacing: '-0.3px' }}>HostelConnect</span>
          </NavLink>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', padding: '6px', borderRadius: '8px', position: 'relative' }}>
            🔔
            <span style={{ position: 'absolute', top: '4px', right: '4px', width: '8px', height: '8px', borderRadius: '50%', background: '#f87171', border: '2px solid #052e16' }} />
          </button>
          <button
            onClick={() => setHamburgerOpen(o => !o)}
            style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #16a34a, #15803d)', border: '2px solid #166534', cursor: 'pointer', color: '#fff', fontWeight: 600, fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
      ...
      <div style={{ display: 'flex', flex: 1, position: 'relative' }}>
        ...
        <aside style={{
          ...
          <div style={{ padding: '20px 0', minWidth: '220px' }}>
            <div style={{ padding: '0 20px 20px', borderBottom: '1px solid #14532d' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, #16a34a, #15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 600, fontSize: '16px', flexShrink: 0 }}>
                  {user?.name?.[0]?.toUpperCase() || 'F'}
                </div>
                <div>
                  <p style={{ margin: 0, color: '#f0fdf4', fontSize: '14px', fontWeight: 600 }}>{user?.name || 'Faculty'}</p>
                  <p style={{ margin: 0, color: '#4ade80', fontSize: '11px' }}>Faculty</p>
                </div>
              </div>
            </div>
              ...
              {facultyNavLinks.map(link => (
                <NavLink key={link.path} to={link.path} onClick={() => setSidebarOpen(false)}
                  style={({ isActive }) => ({
                    ...
                    textDecoration: 'none', color: isActive ? '#4ade80' : '#86efac',
        ...
        <div style={{
          ...
          <div style={{ padding: '24px', minWidth: '260px' }}>
            <div style={{ textAlign: 'center', paddingBottom: '20px', borderBottom: '1px solid #14532d' }}>
              <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'linear-gradient(135deg, #16a34a, #15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '24px', margin: '0 auto 12px' }}>
                {user?.name?.[0]?.toUpperCase() || 'F'}
              </div>
              <p style={{ margin: '0 0 4px', color: '#f0fdf4', fontWeight: 600, fontSize: '15px' }}>{user?.name}</p>
              <p style={{ margin: 0, color: '#4ade80', fontSize: '12px' }}>{user?.email}</p>
              <span style={{ display: 'inline-block', marginTop: '8px', padding: '2px 10px', borderRadius: '20px', background: 'rgba(22,163,74,0.15)', color: '#4ade80', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Faculty</span>
            </div>
            <div style={{ paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {[
                { icon: '👤', label: 'My Profile',    path: '/faculty/profile' },
                { icon: '⚙️', label: 'Settings',      path: '/faculty/settings' },
                { icon: '🔔', label: 'Notifications', path: '/faculty/notifications' },
              ].map(item => (
              ...
              <div style={{ borderTop: '1px solid #14532d', margin: '12px 0' }} />
